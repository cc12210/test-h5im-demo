...
import { login, loginByUserNamePwd, sendSms } from '@/api/login'
...
const onSubmit = async () => {
  ...
  try {
    ...
    const chatToken = token;
        const imToken = token;
        const userID = userid;
        console.log({ chatToken, imToken, userID })
...
const autoLogin = async () => {
  const params = {
    username: "supportlogin001",
    password: "123456",
    platform: 4,
    // devid: crypto.randomUUID(),
  };
  try {
    const res: any = await loginByUserNamePwd(params);
